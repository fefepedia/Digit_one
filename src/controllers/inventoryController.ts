...
export const removeInventoryItem = async (req: Request, res: Response) => {
  try {
    ...
    const remainingItems = await InventoryItem.find({ inventoryId: removedItem.inventoryId });
...
const removeFromSuperInventory = async (inventoryId: mongoose.Types.ObjectId) => {
  try {
    const superInventory = await SuperInventory.findOne({ inventories: inventoryId });
    if (superInventory) {
      superInventory.inventories = superInventory.inventories.filter(i => !i.equals(inventoryId));
...
export const removeInventory = async (req: Request, res: Response) => {
  try {
    ...
    // Reflect this change in the SuperInventory
    const objectId = new mongoose.Types.ObjectId(id);
    await removeFromSuperInventory(objectId);

    return res.status(200).json({ message: 'Inventory removed successfully.' });
  } catch (error) {
    return res.status(500).json({ error: 'An error occurred while removing the inventory.' });
  }
};
